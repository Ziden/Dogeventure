...
namespace UnityEditor.EasyAddressables.Bundler
{
    class EasyAddressableBundler : AnalyzeRule
    {
        public override bool CanFix { get { return true; } }
        public override string ruleName { get { return "EasyAddressables"; } }

        public static bool REMOVE_UNUSED = false;
        public static bool IGNORE_SMALL = false;

        string templateToUse = "Packed Assets";
        string autoGroupPrefix = "(Easy) ";
        public static string FOLDER_NAME = "AutoBundles";
        ...
        string forceLabel = "ForceAddressable";
        ...
        {
            Debug.Log("Refreshing Addressables Rules on Folder " + FOLDER_NAME);
            ...
            var folders = AssetDatabase.GetSubFolders(FOLDER_NAME);
          
            if(!AssetDatabase.IsValidFolder(FOLDER_NAME))
            {
                Debug.LogWarning("Invalid folder " + FOLDER_NAME);
                return results;
            }
            ...
            var allGuids = AssetDatabase.FindAssets(assetFilter, new[] { FOLDER_NAME });
            ...
            string preamble = FOLDER_NAME + "/";
            var expresslyBundled = new HashSet<string>();

            foreach (var folder in folderNames)
            {
                var assetGuids = AssetDatabase.FindAssets(assetFilter, new[] { FOLDER_NAME + "/" + folder });
        ...
        public override void FixIssues(AddressableAssetSettings settings)
        {
            ...
            Debug.Log("Running Organizer Rule");
            Debug.Log("Groups to create: " + groupsToCreate.Count);
            Debug.Log("Groups to remove: " + groupsToRemove.Count);
            Debug.Log("Actions: " + assetActions.Count);
            ...
            ClearAnalysis();
            ClearOurData();
            Debug.Log("Addressables Organized");
        }
        ...
        class RegisterBuildBundleLayout
        {
            ...
            {
                AnalyzeSystem.RegisterNewRule<EasyAddressableBundler>();
            }
        }
    }
}