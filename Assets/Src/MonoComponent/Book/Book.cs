...
public class Book : MonoBehaviour {
    ...
    void Start()
    {
        ...
        InputService.OnAnyButton += OnButton;
    }
    ...
    {
        Main.Services.Scenes.UnloadScene(AssetScene.Book);
    }

    private void AdjustOwnedPages()
    {
        var ownedPages = new List<Sprite>();
        var allPageSprites = bookPages.ToList();
        var maxPages = allPageSprites.Count / 2;
        var owned = Main.Services.Data.Pages();
        for (byte p = 0; p < maxPages; p++)
        {
            var spriteIndex = p * 2;
            if (owned.Contains(p))
            {
                ownedPages.Add(allPageSprites[spriteIndex]);
                ownedPages.Add(allPageSprites[spriteIndex + 1]);
            }
        }
        bookPages = ownedPages.ToArray();
    }
